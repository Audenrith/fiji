 ...
 */
public class MaximumSpotOverlay extends Roi {
	...
	public MaximumSpotOverlay(final Model model, final ImagePlus imp, final Map<String, Object> displaySettings) {
		super(0, 0, imp);
		this.model = model;
		this.imp = imp;
	...
	protected void drawSpot(final Graphics2D g2d, final Spot spot, final double zslice, final int xcorner, final int ycorner, final double magnification) {
		...
		if (dz2 >= radius*radius)
			...
			g2d.fillOval((int) Math.round(xs - 2*magnification), (int) Math.round(zs - 2*magnification), 
					 (int) Math.round(4*magnification), 	(int) Math.round(1));
		else {
