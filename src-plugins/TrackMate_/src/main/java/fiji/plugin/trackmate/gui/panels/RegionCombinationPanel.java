 ...
 */
public class RegionCombinationPanel extends ActionListenablePanel {
	...
	public RegionCombinationPanel(String xKey, List<String> regionNames, List<String> regions) {
		super();
		this.xKey = xKey;
		this.features = regions;
		this.names = regionNames;
		initGUI();
		addFeature();
	}
	...
	public String getXKey() {
		return names.get(jComboBoxXFeature.getSelectedIndex());
	}
	...
	private void addFeature() {
		...
		ComboBoxModel jComboBoxYFeatureModel = new DefaultComboBoxModel(features.toArray());
	 ...
	 */
	private void fireCombineRegionsEvent() {
		...
		ActionEvent combineEvent = new ActionEvent(this, 0, command );
		for (ActionListener listener : actionListeners) {
			listener.actionPerformed(combineEvent);
		}
	}
	...
	private void initGUI() {
		try {
			...
			setLayout(new BorderLayout(0, 0));
			{
				...
				JButton combineButton = new JButton("Combine Regions", PLOT_ICON);
				combineButton.setBounds(80, 27, 140, 40);
				combineButton.addActionListener(new ActionListener() {
					@Override
					public void actionPerformed(ActionEvent arg0) {
						fireCombineRegionsEvent();
					}
				});
				topPanel.add(combineButton);
				combineButton.setFont(FONT.deriveFont(Font.BOLD));
			}
			{
				jLabelXFeature = new JLabel();
				jLabelXFeature.setBounds(10, 93, 170, 13);
				topPanel.add(jLabelXFeature);
				jLabelXFeature.setText("Target Region:");
				jLabelXFeature.setFont(FONT.deriveFont(12));
			}
			{
				ComboBoxModel jComboBoxXFeatureModel = new DefaultComboBoxModel(names.toArray());
				...
				jComboBoxXFeature.setSelectedIndex(names.indexOf(xKey));
			}
			{
				...
				jLabelYFeatures.setText("Regions to combine with:");
