 ...
 */
public class MaximumTrackOverlay extends Roi {
	...
	public MaximumTrackOverlay(final Model model, final ImagePlus imp, final Map<String, Object> displaySettings) {
		super(0, 0, imp);
		this.model = model;
	...
	protected void drawEdge(final Graphics2D g2d, final Spot source, final Spot target,
		...
		final double y0i = source.getFeature(Spot.FRAME);
		final double x1i = target.getFeature(Spot.POSITION_X);
		final double y1i = target.getFeature(Spot.FRAME);
		// In pixel units
